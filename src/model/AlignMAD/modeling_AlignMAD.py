...
from .configuration_AlignMAD import AlignMADConfig
...
@dataclass
class AlignMADModelOutput(BaseTASDModelOutput):
    """
    Model output for AlignMAD.
    """
...
class RQKMeans(nn.Module):
    def __init__(self, config: AlignMADConfig):
        super().__init__()
...
@register_model("AlignMAD")
class AlignMAD(BaseTASDModel):
    def __init__(self, config: AlignMADConfig):
        super().__init__(config)
        self.model = RQKMeans(config)

    def _align_to_period(self, timeslide, timestamp):
        window = self.config.window_size
        timeslide = torch.as_tensor(timeslide)
        timestamp = torch.as_tensor(timestamp)
        n, w = timeslide.shape
        range_ = torch.arange(window, device=timestamp.device)  
        window_start_idx = timestamp[:, 0]
        align_pos = window_start_idx % window
        idx = (range_ - align_pos.unsqueeze(-1)) % window 

        aligned = timeslide.gather(1, idx)
        return aligned
    ...
    def fit(self, train_data, test_data, train_args, processor, **kwargs):
        ...
        timeslide = self._align_to_period(inputs.data[DatasetFeature.TIMESLIDE.value], inputs.data[DatasetFeature.TIMESTAMP.value])
    ...
    def forward(self, timeslide, timestamp, **kwargs):
        timeslide = self._align_to_period(timeslide=timeslide, timestamp=timestamp)
        idx, score = self.model(timeslide, return_dist=True)
        return AlignMADModelOutput(
            score=score[-1],
            idx=idx
        )
