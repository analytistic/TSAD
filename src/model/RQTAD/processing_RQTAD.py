...
class RQTADProcessor(BaseProcessor):
    ...
    def get_point_scores(self, window_scores, window_size, stride, padding_length):
        window_scores = np.asarray(window_scores).flatten()
        num_windows = len(window_scores)
        unwindowed_length = stride * (num_windows - 1) + window_size + padding_length

        diff_cum = np.zeros(unwindowed_length + 1)
        diff_cnt = np.zeros(unwindowed_length + 1)

        begins = np.arange(num_windows) * stride
        ends = begins + window_size
        
        np.add.at(diff_cum, begins, window_scores)
        np.add.at(diff_cum, ends, -window_scores)
        
        np.add.at(diff_cnt, begins, 1)
        np.add.at(diff_cnt, ends, -1)

        cum = np.cumsum(diff_cum[:-1])
        cnt = np.cumsum(diff_cnt[:-1])
